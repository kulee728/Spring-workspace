...
public class ExampleController {
	/*
	 * Model - Spring 에서 데이터 전달 역할을 하는 객체 - org.springframework.ui 패키지
	 * - @SessionAttributes와 함께 사용할 경우 session scope 반환 [기본 사용법]
	 * Model.addAttribute("key" , value); Model.addAttribute("html에 전달가능한 이름", 전달할
	 * 값을 작성);
	 */
	...
	public String ex1(HttpServletRequest req, Model model) {
		...
		/*
		 * model.addAttribute("별칭", 별칭이 아래와 똑같지 않아도 됨); <li th:text="${별칭}">mem 객체</li>
		 * 
		 * model.addAttribute("a", b); <li th:text="${a}">mem 객체</li>
		 * 
		 * 나중에 이름이 다르면 문제가 생겼을 때 찾기 어렵기 때문에 a와 b의 별칭을 동일하게 설정해주는 것이 좋음
		 */
	...
	/*
	 * @PathVariable - 주소 중 일부분을 변수 값 처럼 사용 + 해당 어노테이션으로 얻어온 값은 request scope에 세팅
	 */
	...
	public String ex4(Model model) {
		...
		model.addAttribute("testIf","99999999");
		model.addAttribute("alpha","C");
		return "example/ex4";
	}
	...
	public String ex5(Model model) {
		...
		model.addAttribute("std",new StudentDTO("7899","라이츄",33));
		model.addAttribute("mem",new MemberDTO("77777779","안건혁","44"));
		return "example/ex5";
	}
